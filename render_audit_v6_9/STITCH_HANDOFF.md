# Stitch UI Handoff - v7.0 / v7.1 준비

## 현재 기준

- 현재 앱은 `index.html` Version 7.0 / tag `v7.0` 기준입니다.
- v7.0은 Stitch 003 visual grammar, 문서 기준, 렌더 감사 기준을 닫은 안정 기준입니다.
- v7.1은 003 Stitch 모바일/웹 시안을 실제 앱 UI에 탭별로 이식하는 단계입니다.
- 기존 `v6.9` tag는 `f0fa736 v6.9 UI 정리 최종 회귀 마감`에 남아 있습니다.
- `f0fa736` 이후 v7.0까지의 커밋은 v6.9 post-tag hotfix / renderer audit / QA runner portability / v7.0 기준 정리로 봅니다.
...
`기본세팅`이라는 과거 표기는 실제 폴더명 `기본설정`으로 읽지 않습니다.
`inbody_today`는 현재 로컬 003 세트에 없으므로 구현 기준으로 삼지 않습니다.
...
1. 공통 shell / nav / Settings 톱니바퀴 진입 정리
2. Today 상단 / 갱신 상태 / Coach 중복 정리
3. Records 003 시안 이식
4. 최근 흐름 003 시안 이식
5. InBody 003 시안 이식
6. Settings / Backup 003 시안 이식
7. 전체 모바일 / 웹 screenshot QA 후 v7.1 마감 판단
...
- Recent 3일처럼 데이터가 부족한 상태는 의도적으로 차분하지만, Stitch에게는 저데이터 상태의 빈 화면 / 약한 상태 표현도 함께 보여주는 편이 좋습니다. 1일은 최근 흐름 탭이 아니라 Today / Records 당일 문맥으로 둡니다.
