...
- scoring implementation은 현재 `v8.4_joint_axis_retired_continuous_macro_score_v1`로 구현된 상태다. current score는 rejected thresholded joint 감점축을 제외한 8축이고, carb-fat joint allocation/adaptive target/conditional ranges는 유지한다. 후속 scoring 작업은 REQUIRED_NEXT_GATES와 docs-policy를 확인한 뒤 진행한다.
