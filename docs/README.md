...
- `v8.4_option_c_joint_residual_exact_formula_simulation_2026-07-12.md`
  ...
  - actual audit은 full-backup envelope와 raw goalSnapshot 권한을 score 호출 전에 검사한다. snapshotless/current Settings fallback, raw 숫자 타입 강제변환, 비정상·중복 날짜는 이유별로 제외하고 운동 맥락은 raw snapshot의 production helper 결과로 소유한다. meal `alcoholKcal`/`otherKcal`는 key 부재만 legacy 0으로 허용하고 present-invalid 및 meal/day energy overflow는 score 전에 제외하며, full-day 30% 기준은 production scoring kcal와 맞춘다. routine도 simple 공통 선택지 또는 advanced profile별 production 선택지와 일치해야 한다. `explicit-backup` 경로가 있을 때만 production 8축 + goal/training/source-validity exact/복수 tolerance를 만들고, residual/score를 숨긴 standalone hypothesis-blind packet을 먼저 제공한다. hash-linked reveal은 판단에 사용한 locked packet과 같은 input일 때만 별도 ignored-debug artifact로 열고 packet은 덮어쓰지 않는다. 현재 workspace에는 명시된 backup 경로가 없다.
