...
사용자가 `다음 한 단계만, 충돌 발견 시 멈추고 재판단`이라고 말하면 다음 규칙으로 바로 실행한다.

- 다음 미완료 단계 하나만 진행한다. 단계 안에서도 새 충돌이 나오면 더 밀고 가지 않는다.
- "다음 단계가 남아 있음"은 충돌이 아니다. 충돌은 문서, 실제 코드, 실제 렌더, 테스트, 사용자 최신 의도가 서로 다르게 말할 때다.
- 충돌을 발견하면 먼저 `문서 수정`, `코드 수정`, `설계 재판단`, `사용자 승인 필요` 중 하나로 분류하고 멈춘다.
- 단계가 정상 완료되면 테스트/렌더/문서 확인 범위를 보고하고, 다음 단계는 사용자가 다시 지시할 때까지 시작하지 않는다.
