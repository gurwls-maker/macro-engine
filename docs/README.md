...
- `v8.4_option_c_joint_residual_exact_formula_simulation_2026-07-12.md`
  ...
  - 원래 outcome `MORE_EVIDENCE_REQUIRED`와 production-authority correction outcome `AUTHORITY_CORRECTED_AWAITING_EXPLICIT_BACKUP`은 각 historical 단계의 기록으로 유지한다. 이후 explicit backup actual evidence는 `ACTUAL_EVIDENCE_INSUFFICIENT`로 닫혔고 current joint replacement와 joint-axis removal은 모두 미승인이다.
