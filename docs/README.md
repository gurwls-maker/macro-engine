...
2026-06-15 추가 확정: `가이드 기준`과 `활동량 기준`은 둘로 유지한다. 다만 이것은 예전의 "업무/생활활동은 목표칼로리에 절대 반영하지 않는다" 또는 "두 기준의 목표 칼로리는 언제나 똑같다"는 임시 결론을 되살리는 뜻이 아니다. 다음 산식 작업은 두 기준을 유지한 채 시간 소유권 TDEE, Settings 기본 하루, Today 예외값, Records 스냅샷, Coach, 점수 기준이 어떻게 함께 움직일지 다시 설계해야 한다.

2026-06-15 CG 추가 설계: `가이드 기준`은 Settings의 평소 하루를 기준으로 한 계획 관점, `활동량 기준`은 Today의 실제 업무·생활활동·운동 예외를 반영하는 당일 관점으로 설계한다. 이 단계는 report-only이며 production 목표칼로리/탄단지를 아직 바꾸지 않는다. 다음 production 적용 전에는 외부근거 기반 시간 소유권 TDEE 검증, Records 스냅샷, Coach, 점수, 백업 기준을 함께 닫아야 한다.

2026-06-15 추가 확정: `운동 여부`는 `가이드 기준/활동량 기준`보다 상위에 있는 사용자 모드다. 온보딩이 생기기 전까지는 Settings에서만 바꾸고, 온보딩이 생기면 사용자는 먼저 "운동을 관리 기준에 포함할지"만 고른다. 운동함 모드에서는 운동 프로필/루틴/세션/유산소와 가이드/활동량 관점을 모두 사용할 수 있다. 운동 안 함 모드에서는 운동 관련 입력과 해석을 화면에서 접고, 저장된 운동값은 지우지 않되 계산/렌더에서만 0 또는 fallback으로 처리한다. 같은 백업/내보내기 형식을 유지하며, 숨겨진 값을 0으로 덮어써서 나중에 운동함 모드로 돌아왔을 때 과거 운동 데이터가 사라지는 일을 금지한다.

운동 여부 모드는 Coach/최근 흐름 본문에서 정책 설명처럼 노출하지 않는다. "운동 기록은 현재 관리 기준에 포함하지 않습니다", "일반 권장 범위 안에서 관리합니다", "체성분 변화와 식단 기록을 함께 봅니다" 같은 문구는 사용자가 해야 할 행동이 아니라 앱 내부 정책 설명이므로 Settings 도움말, `?` 툴팁, 온보딩, 기준문서에만 둔다. Coach/최근 흐름은 사용자가 지금 볼 수 있는 상태와 다음 행동만 말한다.

2026-06-15 코드 영향 감사: `docs/v8_운동여부_코드영향감사_2026-06-15.txt`를 추가했다. 현재 코드에는 신규 운동 여부 모드가 없고, 기존 `mode` / `generalAdvancedSettings`는 다른 의미라 재사용하면 안 된다. 구현은 저장값과 계산 유효값을 분리하는 방식으로 해야 하며, 다음 구현 전에는 운동 안 함 모드에서 `가이드 기준/활동량 기준` 토글을 그대로 보여줄지와 InBody 해석 범위를 어디까지 둘지 결정해야 한다.
